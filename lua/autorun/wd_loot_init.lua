...
if SERVER then
    ...
    -------------------------------------------------
    -- Roll Multiple Loot Items
    -------------------------------------------------

    local function RollLootTable(tier)
...
hook.Add("CanTool", "WD_BlockToolgunLoot", function(ply, trace, tool)
    if IsValid(trace.Entity) and trace.Entity.WD_IsLoot then
        return false
    end
end)
   ...
   local function ApplyLoot(ent, tier)
    ...
    ent.WD_IsLoot = true
    ent.WD_Tier = tier
    ent.WD_LootItems = RollLootTable(tier) or {}
    ...
    -- Block ALL damage
    ent:AddCallback("OnTakeDamage", function()
        return true
    end)

    -- Extra protection against explosions
    ent:AddCallback("EntityTakeDamage", function()
        return true
    end)
    ...
    if not ent.WD_IsLoot then return end
    if not ent:GetNWBool("WD_HasLoot") then return end
...
net.Receive("WD_TakeLoot", function(_, ply)
    ...
    if not IsValid(ent) then return end
    if not ent.WD_IsLoot then return end
    if not ent.WD_LootItems then return end

    -- 🔒 Prevent spam clicks (per player)
    ...
    -- 🔒 Validate item actually exists in container
    ...
    -- If item not found, stop (prevents duping)
    if not foundIndex then return end

    -- Remove FIRST, then give weapon
    table.remove(ent.WD_LootItems, foundIndex)

    -- Extra safety: don't give if player already has it
    if not ply:HasWeapon(item) then
        ply:Give(item)
    end
...
if CLIENT then
    ...
    for tier, data in pairs(WD_Loot.Tiers) do
        ...
        for _, ent in ipairs(ents.FindByClass("prop_physics")) do
            ...
            -- 🔥 Distance check (adjust if you want)
            if plyPos:DistToSqr(ent:GetPos()) > 250000 then continue end -- 500 units
    ...
    if IsValid(WD_LootFrame) then return end
...
icon:SetTooltip(item.class)
    ...
    net.Start("WD_TakeLoot")
        net.WriteEntity(ent)
        net.WriteString(item.class) -- FIXED
    net.SendToServer()
...
hook.Add("Think", "WD_LootRefreshThink", function()
    ...
    for _, ent in ipairs(ents.GetAll()) do
        ...
        if not ent.WD_IsLoot then continue end
        if ent:GetNWBool("WD_HasLoot") then continue end
        if not ent.WD_NextRefresh then continue end
